import image1 from "../images/image.jpg";
import image2 from "../images/image2.jpg";
import image3 from "../images/image3.jpg";
import image4 from "../images/image4.jpg";
import imagei1 from "../images/imagei1.jpg";
import imagei2 from "../images/imagei2.jpg";
import imagei3 from "../images/imagei3.jpg";
import imagei4 from "../images/imagei4.jpg";
import imagei5 from "../images/imagei5.jpg";
import imagei6 from "../images/imagei6.jpg";
import imagei7 from "../images/imagei7.jpg";
import imagei8 from "../images/imagei8.jpg";
import imagei9 from "../images/imagei9.jpg";
import imagei10 from "../images/imagei10.jpg";
import imagei11 from "../images/imagei11.jpg";
import imagei12 from "../images/imagei12.jpg";
import imagei13 from "../images/imagei13.jpg";
import imagei14 from "../images/imagei14.jpg";
import imagei15 from "../images/imagei15.jpg";

const homeData = [
  {
    src: image1,
    title: "Deals",
    text:
      "This tools uses pricing information from our buy and sell vendors to identify potential deals",
  },
  {
    src: image2,
    title: "Bulk lookup",
    text:
      "Searches upto 1,000 books per day. We pull the information on the fly,to be patient if you have a large list.",
  },
  {
    src: image3,
    title: "Sell your books",
    text: "you can sell your books at releavent price",
  },
  {
    src: image4,
    title: "Toppers and Top teachers",
    text: "You wil get study material of top teachers and students",
  },
];

const que = [
  {
    title: "It’s a Wrap! Creative Ideas for Wrapping Books...",
    text:
      "Giving an attractively wrapped book this holiday season doesn’t have to be costly. With a little extra ingenuity and a few items you might have around the house you can put a personal touch on a gift to make it more meaningful. And luckily, books are one of the easier items to wrap!",
  },
  {
    title: "Holiday Gift Guide: Books and Other Gifts for Book Lovers...",
    text:
      "It’s that time of year when your thoughts turn to finding that special holiday gift for friends and family who are book lovers. But what should be a fun, festive task can quickly become overwhelming. We’ve done some of the leg work for you compiling a collection of 14 unique, niche book selections and other gifts reading fans will appreciate. And don’t forget the BEST way to be sure you are getting the lowest prices on books is to use the handy search tools on Bookscouter.com –we do all the work of searching for you!",
  },
  {
    title: "What’s Cookin’? Cookbook Values Can Heat Up...",
    text:
      "It’s the time of year for a lot of holiday cooking and baking. As you dust off an old cookbook and consider letting it go to make extra space, you might want to first check BookScouter.com to see the current value. If it’s really old, you may want to have it appraised by an antique book dealer. You might decide to hold on to it and then pass it down to a family member. Perhaps you come across a rare find and decide to sell it. Or you might decide that a vintage cookbook would make a thoughtful gift for a cook or collector you know.",
  },
  {
    title: "Common Mistakes Students Make With Money...",
    text:
      "Start by determining a budget. Items to include in your budget: any monthly bills, expenses like groceries, miscellaneous charges, and a fund to cover unexpected expenses. Know how much ‘fun money’ you can spend each month and when it’s gone, stop spending! It may sound simple but you need to have a handle on your income and be able to distinguish needs from wants.Now is the best time to learn smart budgeting habits that can benefit you beyond college. Here are some tips to learn how to set a budget or try a free and easy app like Mint that automatically helps set up a budget you can customize.",
  },
  {
    title: "Five Ideas to Find Your Next Great Read...",
    text:
      "Itching to read a good page-turner? Just finish the final book in a multi-volume series? Need to make a vetted suggestion for your next book club? Here are five ideas on how to find your next great read",
  },
];

const item = [
  {
    src: imagei1,
    title:
      "Python Crash Course, 2nd Edition: A Hands-On, Project-Based Introduction to Programming",
    author: "Matthes, Eric",
    price: "120rs",
  },
  {
    src: imagei2,
    title: "The Art of Computer Programming, Volumes 1-4A Boxed Set",
    author: "Knuth, Donald",
    price: "130rs",
  },
  {
    src: imagei3,
    title:
      "The Self-Taught Programmer: The Definitive Guide to Programming Professionally",
    author: "Althoff, Cory",
    price: "150rs",
  },
  {
    src: imagei4,
    title: "Fundamentals of Software Architecture: An Engineering Approach",
    author: "Richards, Mark; Ford, Neal",
    price: "180rs",
  },
  {
    src: imagei5,
    title:
      "Software Design for Flexibility: How to Avoid Programming Yourself into a Corner",
    author: "Hanson, Chris; Sussman, Gerald Jay",
    price: "200rs",
  },
  {
    src: imagei6,
    title: "Learn C++ Quickly: A Complete Beginner’s Guide to Learning C++",
    author: "Quickly, Code",
    price: "180rs",
  },
  {
    src: imagei7,
    title:
      "Building Thinking Classrooms in Mathematics, Grades K-12: 14 Teaching Practices for Enhancing Learning",
    author: "Liljedahl, Peter",
    price: "220rs",
  },
  {
    src: imagei8,
    title: "Math Without Numbers",
    author: "Beckman, Milo",
    price: "230rs",
  },
  {
    src: imagei9,
    title:
      "The EduProtocol Field Guide Math Edition: 15 Student-Centered Lesson Frames for Math Mastery",
    author: "Lisa; Ruesch, Jeremiah",
    price: "240rs",
  },
  {
    src: imagei10,
    title: "Lost in Math: How Beauty Leads Physics Astray",
    author: "Hossenfelder, Sabine",
    price: "250rs",
  },
  {
    src: imagei11,
    title:
      "Weapons of Math Destruction: How Big Data Increases Inequality and Threatens Democracy",
    author: "O'Neil, Cathy",
    price: "300rs",
  },
  {
    src: imagei12,
    title: "Math Skillbuilders (Grades 2 - 3) (Step Ahead)",
    author: "Golden Books",
    price: "320rs",
  },
  {
    src: imagei13,
    title: "Basic Physics: A Self-Teaching Guide, 3rd Edition",
    author: "Kuhn, Karl F.; Noschese, Frank",
    price: "330rs",
  },
  {
    src: imagei14,
    title: "The Essential Physics of Medical Imaging",
    author: "Bushberg PhD, Jerrold T.; Seibert PhD",
    price: "340rs",
  },
  {
    src: imagei15,
    title: "The Physics Book: Big Ideas Simply Explained",
    author: "DK",
    price: "350rs",
  },
];

export { homeData, que, item };
